...
class RegistrationBloc extends Bloc<RegistrationEvent, RegistrationState> {
  ...
  RegistrationBloc()
    ...
    on<RegistrationEmailChanged>(_onEmailChange);
    on<RegistrationEmailFocusLost>(_onEmailFocusLost);
    on<RegistrationPasswordChanged>(_onPasswordChange);
    on<RegistrationPasswordFocusLost>(_onPasswordFocusLost);
    on<RegistrationPasswordConfirmationChanged>(_onPasswordConfirmationChange);
    on<RegistrationPasswordConfirmationFocusLost>(
        _onPasswordConfirmationFocusLost);
    on<RegistrationNameChanged>(_onNameChange);
  ...
  FutureOr<void> _onEmailChange(final RegistrationEmailChanged event,
      final Emitter<RegistrationState> emit) {
    _email = event.email;
  ...
  FutureOr<void> _onPasswordChange(final RegistrationPasswordChanged event,
      final Emitter<RegistrationState> emit) {
  ...
  FutureOr<void> _onPasswordConfirmationChange(
  ...
  FutureOr<void> _onNameChange(final RegistrationNameChanged event,
      final Emitter<RegistrationState> emit) {
    _name = event.name;
  ...
  FutureOr<void> _onCreateAccount(final RegistrationCreateAccount event, final Emitter<RegistrationState> emit) async {
    _highlightEmailError = true;
    _highlightPasswordError = true;
    _highlightPasswordConfirmationError = true;
    _highlightNameError = true;
  ...
  RegistrationPasswordError? _validatePassword() {
    ...
    if (_password.length < 6) {
      return RegistrationPasswordError.toShort;
    }
